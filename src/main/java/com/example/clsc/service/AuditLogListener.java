...
public class AuditLogListener {
    ...
    @Autowired
    public AuditLogListener(AuditLogRepo auditLogRepo, AuditLogRepo missingLogRepo) {
        this.auditLogRepo = auditLogRepo;
        ...
        try {
            ...
            if (previousLogOpt.isPresent()) {
                ...
                if (prevUpdatedAt != null && currUpdatedAt != null) {
                    if (compareTimestamps(prevUpdatedAt, currUpdatedAt) >= 0) {
                        // Normal update case
                    ...
                    } else if(compareTimestamps(prevUpdatedAt, currUpdatedAt) < 0) {
    ...
    private AuditLog createMissingAuditLog(AuditEvent message, AuditLog previousLog) throws Exception {
        ...
        return missing;
         ...
         /* Compares two LocalDateTime values.
         *
         * @param a the first timestamp
         * @param b the second timestamp
         * @return -1 if a < b, 1 if a > b, 0 if equal, -2 if either is null
         */
        private static int compareTimestamps(LocalDateTime a, LocalDateTime b) {
            if (a == null || b == null) {
                return -2; // or throw an exception if you prefer
            }
            return a.compareTo(b); // returns -1, 0, 1
        }
    }
